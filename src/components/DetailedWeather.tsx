...
export const DetailedWeather = () => {
  ...
  const cityTimeOffset = detailed["timezone"]
  const userTime = new Date().toString().split(' ')[4].split(':')
  const userTimeArray = (userTime as unknown as number[])
  const userSecNumber = userTimeArray[0] * 3600 + userTimeArray[1] * 60 + userTimeArray[2] * 1
  const cityTime3hoursGap = (userSecNumber - userTimeOffset + cityTimeOffset) / 3600 / 3
  ...
  const startHourForecast = gapNumber * 3
  ...
  return (
    <Card sx={{ backgroundColor: '#DCDCDC', minWidth: 310 , borderRadius: 2 }}>
      <CardContent>
        <div>
          <div style={{color: '#19181A'}}>{currentCity as unknown as string}</div>
